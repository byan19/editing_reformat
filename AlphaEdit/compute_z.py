...
def compute_z(
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        if flatness_loss:
            ...
            flat_loss = flat_loss_lambda  * flat_loss.mean()
...
def compute_z_memeory_leaking_sovler_trail1(
        model: AutoModelForCausalLM,
        tok: AutoTokenizer,
        request: Dict,
        hparams: AlphaEditHyperParams,
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        grad_edit = torch.autograd.grad(loss, delta)[0]
        ...
        if flatness_loss:
            ...
            output = model(**input_tok, output_hidden_states=True)
            #hidden_states = output.hidden_states[layer + 1: layer + 3]
            hidden_states = output.hidden_states[layer + 1: ]
            ...
            #noise_hidden_states = model(**input_tok, output_hidden_states=True).hidden_states[layer + 1: layer + 3]
            noise_hidden_states = model(**input_tok, output_hidden_states=True).hidden_states[layer + 1: ]
            '''
            ...
            grad_flat = torch.autograd.grad(flat_loss, delta)[0]
            delta.grad = grad_edit + grad_flat
        else:
            flat_loss = torch.tensor([0.0]).mean()
    
            delta.grad = grad_edit
