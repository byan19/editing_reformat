...
def apply_AlphaEdit_Hessian_to_model(
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        if hparams.hessian_type == 'origin':
            ...
            if holder_matrix.norm(2).item() > largest_norm:
                largest_norm = holder_matrix.norm(2).item()
            hessian[i, :, : ] += fisher_matrix.cpu()/largest_norm + 0.01 * torch.eye(4096).unsqueeze(0).cpu()
        ...
        elif hparams.hessian_type == 'soft_largest_max_norm':
            if holder_matrix.max().item() > largest_norm:
                largest_norm = holder_matrix.max().item()
                hessian[i, :, :] += fisher_matrix.cpu() / largest_norm
            else:
                hessian[i, :, :] += fisher_matrix.cpu() / (holder_matrix.max().item())
